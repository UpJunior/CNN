...
def softmax_layer(x):
    ...
    return h_sf
def getbatch_xy(num):
    path = "C:/Users/Knight/Desktop/Image_Process/VGG/VGG-data/train/"
    image_featuremap = []
    predict_arr = []
...
with tf.Session() as sess:  
	...
	for index in range(1):
	    batch_x , batch_y = sess.run(getbatch_xy(1))
	    # correct_prediction = tf.equal(predict , batch_y)
	    # accuracy = tf.reduce_mean(tf.cast(correct_prediction , tf.float32))
	    # train_accuracy = accuracy.eval(feed_dict = {
	    # 		inputRGB: batch_x, classf: batch_y
	    # 	})
	   
	    print (batch_y)
	    # print (index , train_accuracy)
